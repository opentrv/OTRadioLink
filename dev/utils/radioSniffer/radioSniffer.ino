...
void setup()
{
    ...
    OTV0P2BASE::serialPrintAndFlush(F("\r\Radio PacketSniffer: "));
    OTV0P2BASE::LED_HEATCALL_ON();
...
//    else { OTV0P2BASE::LED_HEATCALL_OFF(); }
    ...
    DEBUG_SERIAL_PRINTLN();
    // print out rxed message in hex and ascii for inspection.
    if(0 != rxed)
    {
        OTV0P2BASE::LED_HEATCALL_ON();
        DEBUG_SERIAL_PRINT(": ");
        ...
        uint8_t buf[len + 1];
        memcpy(buf, (const void *)PrimaryRadio.peekRXMsg(), len);
        buf[len] = '\0';
        for(auto ptr = &buf[0]; 0 != *ptr; ++ptr) {
          DEBUG_SERIAL_PRINTFMT(*ptr, HEX);
          DEBUG_SERIAL_PRINT(" ");
        }
        DEBUG_SERIAL_PRINTLN();
        DEBUG_SERIAL_PRINT("         ");
