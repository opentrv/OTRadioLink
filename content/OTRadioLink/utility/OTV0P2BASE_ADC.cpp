...
//   * samples  maximum number of samples to take (if one, nap() before); strictly positive
// Sets sleep mode to SLEEP_MODE_ADC, and disables sleep on exit.
uint16_t _analogueNoiseReducedReadM(const uint8_t, int8_t)
{
    // Start a conversion
    ADC_Start(ADC0, adcStartSingle);
    // wait for conversion to finish
    while ((ADC0->STATUS & ADC_STATUS_SINGLEDV) == 0);
    return ((uint16_t) ADC_DataSingleGet(ADC0));
}
...
uint16_t analogueNoiseReducedRead(uint8_t, uint8_t) { return _analogueNoiseReducedReadM(0); }
