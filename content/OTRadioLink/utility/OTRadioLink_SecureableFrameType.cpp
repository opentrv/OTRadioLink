...
// Not ISR-safe.
void loadRaw3BytePersistentTXRestartCounterFromEEPROM(uint8_t *const loadBuf)
    {
...
bool getPrimarySecure6BytePersistentTXMessageCounter(uint8_t *const buf)
    ...
    // Security optimisation: if initialising and persistent/restart part is all zeros
    // then force it to an entropy-laden non-zero value that still leaves most of the lifetime.
    // Else simply increment it as per the expected restart counter behaviour.
    // NOTE: at the very list the restart counter must be incremented here.
    if(doInitialisation)
        {
