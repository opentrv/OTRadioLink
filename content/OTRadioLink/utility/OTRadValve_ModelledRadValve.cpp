...
//V0P2BASE_DEBUG_SERIAL_PRINTLN_FLASHSTRING("under temp");
    ...
    // (possibly because of sudden temperature drop already away from near target)
    // AND IF temperature above minimum frost safety threshold
    // and temperature is currently falling
    // and temperature fall over a few minutes is large
    // THEN attempt to turn down slowly if not inhibited from turning down (else stay as-is).
    ...
    if((adjustedTempC > MIN_VALVE_TARGET_C) &&
       ...
       (getRawDelta(MIN_WINDOW_OPEN_TEMP_FALL_M) <= -(int)MIN_WINDOW_OPEN_TEMP_FALL_C16))
        {
        // Close the valve a at least little if possible.
        if((valvePCOpen > 0) && !dontTurndown())
          { return(valvePCOpen - 1); }
        // Else avoid opening the valve.
        return(valvePCOpen);
        }
