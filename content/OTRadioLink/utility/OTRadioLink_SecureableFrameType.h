...
//        uint8_t roundUpTo16s(uint8_t s) { return((s + 15) & 0xf0); }
    ...
    static const uint8_t messageCounterBytes = 6;
    // Returns true on success and fills the supplied 6-byte array.
    // Returns false on failure, for example because the counter has reached its maximum value.
    bool getPrimaryTXMessageCounter(uint8_t *buf);
